<script lang="ts">
 ...
 let reconciliation = '', amount = 0, confirmed = false;
 ...
 async function load(reset = false) {
  if (!id || loading) return; const target=id; loading=true; error='';
  try { const [p,d]=await Promise.all([api.observations(target,task,chapter,offset),api.observationDiagnostics(target)]); if (id!==target) return; page=p;findings=d.findings;if (reset || !draft) resetPolicy(); }
  catch(e){if(id===target) error=String(e);} finally{loading=false;}
 }
 ...
 onMount(()=>{let alive=true;let last=0;api.listProjects().then(p=>{if(!alive)return;projects=p.projects;id=new URLSearchParams(location.hash.split('?')[1]??'').get('project')??projects[0]?.id??'';void load(true);}).catch(e=>error=String(e));const stop=recentEvents.subscribe(events=>{const e=events[0];if(e&&e.id!==last&&e.project===id&&e.type==='observability.changed'){last=e.id;void load();}});const timer=setInterval(()=>void load(),10000);return()=>{alive=false;stop();clearInterval(timer);};});
</script>

...
p=root/'web/src/pages/Observability.svelte';s=p.read_text();assert s.count('if(e&&e.id!==last')==1;s=s.replace('if(e&&e.id!==last',"if(e&&typeof e.id==='number'&&e.id!==last")
old='<td>{money(a.cost_micros,a.currency)}<br/>{a.cost_source}</td>';assert s.count(old)==1;s=s.replace(old,'<td><span>{money(a.cost_micros,a.currency)}</span><br/><span>{a.cost_source}</span></td>');p.write_text(s)

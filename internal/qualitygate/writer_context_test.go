...
func TestWriterContextPrecedesModelCallAndReplay(t *testing.T) {
	...
	if !strings.Contains(invoker.payload, "PINNED-CONTEXT") {
		t.Fatal("compiler output absent from model payload")
	}
	if _, err := writer.Write(context.Background(), req); err != nil {
		t.Fatal(err)
	}
	if invoker.calls != 1 {
		t.Fatal("completed call did not replay")
	}
	...
	if invoker.calls != 1 {
		t.Fatal("compiler failure reached provider")
	}
}

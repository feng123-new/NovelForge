...
func (r *Repository) CompileWriterContext(ctx context.Context, req qualitygate.WriterRequest, totalTokens int) (json.RawMessage, error) {
	if req.ProjectID == "" || req.Chapter < 1 || req.Plan.Chapter != req.Chapter { return nil, fmt.Errorf("writer context identity is invalid") }
	if err := req.Plan.Validate(); err != nil { return nil, err }
	if totalTokens <= 0 { totalTokens = 25000 }
	...
	for _, f := range ledgerContext.Foreshadows {
		...
		if f.Mandatory { item.Layer = contextcompiler.LayerTruth; item.Requirement = contextcompiler.RequirementCriticalForeshadow; item.Priority = 1000 }
	...
	if err != nil { return nil, err }
	hasPOV := false
	for _, item := range truthItems { if item.Requirement == contextcompiler.RequirementPOVCharacterState { hasPOV = true } }
	...
	all := func(layer contextcompiler.Layer, stage contextcompiler.RetrievalStage) contextcompiler.ItemProvider {
		return contextcompiler.ProviderFunc(func(context.Context, contextcompiler.Request) ([]contextcompiler.Item, error) {
			...
			for _, item := range items {
				if item.Layer == layer && (stage == contextcompiler.StageNone || item.Stage == stage) { out = append(out, item) }
			}
			return out, nil
		})
	}
	providers := contextcompiler.Providers{
		...
		Narrative: contextcompiler.ProviderFunc(func(context.Context, contextcompiler.Request) ([]contextcompiler.Item, error) {
			...
			for _, item := range pinned { if item.Layer == contextcompiler.LayerNarrative { out = append(out, item) } }
			return out, nil
		}),
	...
	return json.Marshal(struct { Text string `json:"text"`; ContextSHA string `json:"context_sha"`; UsedTokens int `json:"used_tokens"` }{compiled.Text, compiled.ContextSHA, compiled.Diagnostics.UsedTokens})
}
...
type povTruthReader struct { store contextcompiler.TruthStateReader; pov string }
func (p povTruthReader) State(ctx context.Context, q truthstore.StateQuery) (truthstore.StatePage, error) {
	...
	for _, fact := range page.Facts {
		...
		if strings.Contains(subject, "secret") || strings.Contains(predicate, "secret") { continue }
		if (strings.Contains(subject, "knowledge") || strings.Contains(predicate, "knowledge") || strings.Contains(predicate, "knows")) && fact.SubjectID != p.pov { continue }
...
func (p recentContextReader) Collect(ctx context.Context, req contextcompiler.Request) ([]contextcompiler.Item, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Kind, &item.Title, &item.Content, &item.SourceChapter, &item.SourceVersion); err != nil { return nil, err }
		item.ID = "recent:" + item.ID; item.Layer = contextcompiler.LayerRecent; item.Priority = 900

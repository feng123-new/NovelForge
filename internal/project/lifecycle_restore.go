...
type LifecycleRestoreResult struct {
	...
	JobsResumed           bool    `json:"jobs_resumed"`
}
...
func (r *Repository) RestoreLifecycle(ctx context.Context, data []byte) (LifecycleRestoreResult, error) {
	...
	m, files, err := lifecycle.Unpack(data)
	if err != nil {
		return result, err
	}
	idBytes, err := hex.DecodeString(m.ProjectID)
	if err != nil || len(idBytes) != 16 {
		return result, lifecycle.ErrInvalid
	}
	var meta Metadata
	d := json.NewDecoder(bytes.NewReader(files[projectMetadataRelative]))
	d.DisallowUnknownFields()
	if d.Decode(&meta) != nil || meta.ID != m.ProjectID || meta.Title != m.Title || meta.FormatVersion != m.Format || m.Format < 1 || m.Format > CurrentFormatVersion {
		return result, lifecycle.ErrInvalid
	}
	if validateCreateInput(CreateInput{Title: meta.Title, TargetWords: meta.TargetWords, TargetChapters: meta.TargetChapters, WordsPerChapter: meta.WordsPerChapter}) != nil {
		return result, lifecycle.ErrInvalid
	}
	if meta.Status != StatusActive && meta.Status != StatusArchived {
		return result, lifecycle.ErrInvalid
	}
	...
	private, err := safeLifecycleDirectory(r.workspace, ".novelforge")
	if err != nil {
		return result, err
	}
	...
	staging, err := os.MkdirTemp(private, "restore-")
	if err != nil {
		return result, err
	}
	...
	for name, b := range files {
		...
		if err = ensureChildPath(staging, p); err != nil {
			return result, err
		}
	...
	if err = initializeLayout(staging); err != nil {
		return result, err
	}
	// Updates use the normal checksum/backup runner, after schema verification.
	if err = r.initializeProjectDatabase(ctx, staging); err != nil {
		return result, err
	}
	if err = writeJSONAtomic(filepath.Join(staging, projectConfigRelative), defaultProjectConfig(), 0600); err != nil {
		return result, err
	}
	if err = os.WriteFile(filepath.Join(staging, ".novelforge", "restore-origin.sha256"), []byte(digest), 0600); err != nil {
		return result, err
	}
	if err = checkpointProjectDatabase(ctx, staging); err != nil {
		return result, err
	}

	...
	for _, dir := range []string{".novelforge", ".ainovel"} {
		for _, name := range []string{dir, filepath.Join(dir, "config.json")} {
			...
			if err != nil && !errors.Is(err, os.ErrNotExist) {
				return bootstrap.Config{}, errors.New("project model configuration is unreadable")
			}
			if err == nil && info.Mode()&os.ModeSymlink != 0 {
				return bootstrap.Config{}, errors.New("project model configuration cannot be a symlink")
			}
		}
	}
	result, err := bootstrap.LoadNovelForgeConfig("", entry.Root, explicit)
	if err != nil {
		return bootstrap.Config{}, errors.New("project model configuration could not be loaded")
	}

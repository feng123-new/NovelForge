...
func (r *Repository) OpenAuthoring(ctx context.Context, id string) (*authoring.Store, error) {
	entry, err := r.find(id)
	if err != nil {
		return nil, err
	}
	...
	return authoring.OpenExisting(filepath.Join(entry.Root, projectDatabaseRelative))
}
func shortAuthoringQuery(query string) string {
...
func (r *Repository) authoringSelection(ctx context.Context, id, scope, role, query string, chapter int, pov string) (authoring.Selection, error) {
	s, err := r.OpenAuthoring(ctx, id)
	if err != nil {
		return authoring.Selection{}, err
	}
	defer s.Close()
	return s.Select(ctx, scope, role, shortAuthoringQuery(query), chapter, pov)
}
func selectionItems(s authoring.Selection) []contextcompiler.Item {
	...
	for _, e := range s.Entries {
		...
		switch e.Kind {
		case "skill":
			item.Content = "Writing-method instructions; subordinate to system, accepted facts and POV boundaries:\n" + e.Markdown
			item.Mandatory = true
		case "style":
			item.Content = "STYLE EXAMPLE ONLY. Do not import its plot, characters or facts:\n" + e.Markdown
		case "knowledge":
			item.Layer = contextcompiler.LayerHistorical
			item.Stage = contextcompiler.StageStructured
			item.Content = "EXTERNAL REFERENCE ONLY. Not canon and not permission for POV knowledge:\n" + e.Markdown
		}
		out = append(out, item)
	}
...
func authoringProviders(items []contextcompiler.Item) contextcompiler.Providers {
	filter := func(layer contextcompiler.Layer) contextcompiler.ItemProvider {
...
func compileAuthoringSelection(ctx context.Context, id string, chapter int, pov string, s authoring.Selection, extra ...contextcompiler.Item) (json.RawMessage, error) {
	result, err := contextcompiler.New(authoringProviders(append(selectionItems(s), extra...)), nil).Compile(ctx, contextcompiler.Request{ProjectID: id, Chapter: chapter, POVEntityID: pov, TotalTokens: 6000, RecentChapterCount: 3, Budget: contextcompiler.DefaultBudgetConfig()})
	if err != nil {
		return nil, err
	}
...
func (r *Repository) CompilePlanningSkills(ctx context.Context, id, scope, query string) (json.RawMessage, error) {
	sel, err := r.authoringSelection(ctx, id, scope, "planning", query, 0, "")
	if err != nil {
		return nil, err
	}
	return compileAuthoringSelection(ctx, id, 0, "", sel)
}
...
func (r *Repository) CompileEditorialContext(ctx context.Context, req qualitygate.EditorRequest) (json.RawMessage, []string, error) {
	...
	for _, finding := range report.Findings {
		notes = append(notes, "[Advisory style] "+finding.Message)
	}
	// A compact model-facing subset is still placed inside the compiler, never appended after it.
	modelReport := report
	if len(modelReport.Findings) > 8 {
		modelReport.Findings = append([]authoring.Finding(nil), modelReport.Findings[:8]...)
		modelReport.Truncated = true
	}
	raw, err := json.Marshal(modelReport)
	if err != nil {
		return nil, nil, err
	}
...
func (r *Repository) AuthoringLint(ctx context.Context, id string, chapter int, text string, rules authoring.Rules) (authoring.Report, error) {
	if chapter < 1 || chapter > 1000 || len(text) > 1<<20 || rules.Validate() != nil {
		return authoring.Report{}, authoring.ErrValidation
	}
	db, err := r.autopilotDatabase(ctx, id)
	if err != nil {
		return authoring.Report{}, err
	}
	defer db.Close()
	rows, err := db.QueryContext(ctx, `SELECT substr(v.content,1,48000) FROM chapter_active_finals a JOIN chapter_versions v ON v.id=a.version_id WHERE a.project_id=? AND a.chapter<? ORDER BY a.chapter DESC LIMIT ?`, id, chapter, rules.PreviousChapters)
	if err != nil {
		return authoring.Report{}, err
	}
	defer rows.Close()
	...
	return rules.Evaluate(text, previous), nil
}

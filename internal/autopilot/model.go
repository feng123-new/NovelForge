...
func (in Input) Validate() error {
	...
	for k, v := range in.ModelProfile {
		if len(k) > 64 || len(v) > 256 {
			return errors.New("invalid model profile")
		}
	}
	if strings.TrimSpace(in.Idea) == "" || len(in.Idea) > 40000 || len(in.Style) > 16000 || len(in.Language) > 80 || in.StartChapter < 1 || in.TargetChapter < in.StartChapter || in.TargetChapter > 1000 || in.ReviewEvery < 0 || in.ReviewEvery > 100 || in.MaxRewrites < 0 || in.MaxRewrites > 5 || in.MaxRetries < 0 || in.MaxRetries > 5 || in.WordsPerChapter < 0 || in.WordsPerChapter > 20000 {
		return errors.New("invalid bounded autopilot input")
	}
	return nil
}
...
func (f Foundation) Validate(target int) error {
	...
	for _, c := range f.Characters {
		...
		seen[c.ID] = true
		found = found || c.ID == f.POV
	}
	if !found {
		return errors.New("foundation POV must identify a character")
	}
	for _, rule := range f.WorldRules {
		if rule == "" || len(rule) > 2000 {
...
func Identity(project, key string) string {
	h := sha256.Sum256([]byte(project + "\x00" + key))
...
type Engine interface {
	Step(context.Context, Job) (Job, error)
}
type Failure struct {
	Code      string
	Retryable bool
...
func Decode(data []byte, target any) error {
	if len(data) == 0 || len(data) > 256*1024 {
		return errors.New("invalid structured output size")
	}

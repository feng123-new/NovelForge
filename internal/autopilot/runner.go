...
func Start(store *Store, engine Engine, acquire func(context.Context, string) (io.Closer, error)) (*Runner, error) {
	...
	if err != nil || !ok {
		lock.Close()
		return nil, errors.New("autopilot workspace already has a worker")
	}
...
func (r *Runner) Close() error {
	...
	r.once.Do(func() { r.ready.Store(false); r.cancel(); <-r.done })
	return nil
}
func (r *Runner) loop(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			if !errors.Is(err, ErrNotFound) && !errors.Is(err, ErrConflict) && ctx.Err() == nil {
				return
			}
			select {
			case <-ctx.Done():
		...
		_, err = r.Store.Finish(finishCtx, job, after, stepErr)
		cancel()
...
func (r *Runner) step(ctx context.Context, j Job) (next Job, err error) {
	...
	defer func() {
		if recover() != nil {
			next = j
			err = Stop("AUTOPILOT_PANIC")
		}
	}()
	lease, err := r.Acquire(ctx, j.ProjectID)
	if err != nil {
		return j, Retry("PROJECT_BUSY")
	}
	defer lease.Close()
	...
	current, err := r.Store.Get(callCtx, j.ID)
	if err != nil {
		return j, err
	}
	if current.Control != "" {

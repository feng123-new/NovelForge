...
func New(config bootstrap.Config) (*Runtime, error) {
	...
	for name, provider := range config.Providers {
		...
		if strings.HasPrefix(key, "${") && strings.HasSuffix(key, "}") {
			...
			if !exists || strings.TrimSpace(value) == "" { return nil, errors.New("quality provider credential environment variable is unset") }
			provider.APIKey = value
			config.Providers[name] = provider
		}
	}
	if err := config.ValidateBase(); err != nil { return nil, errors.New("quality model configuration is invalid") }
	models, err := bootstrap.NewModelSet(config)
	if err != nil { return nil, errors.New("quality model client could not be created") }
...
func (r *Runtime) Invoke(ctx context.Context, operation string, payload []byte) ([]byte, qualitygate.ModelUsage, error) {
	if r == nil || r.models == nil { return nil, qualitygate.ModelUsage{}, errors.New("quality runtime is not configured") }
	prompt, role, err := operationPrompt(operation)
	if err != nil { return nil, qualitygate.ModelUsage{}, err }
	if !json.Valid(payload) { return nil, qualitygate.ModelUsage{}, errors.New("quality request must be JSON") }
	...
	if model == nil { return nil, usage, errors.New("quality role model is unavailable") }
	effort := r.config.ReasoningEffort
	if rc, ok := r.config.Roles[role]; ok && rc.ReasoningEffort != "" { effort = rc.ReasoningEffort }
	...
	if err != nil {
		...
		if errors.As(err, &netErr) { retryable = netErr.Timeout() }
		if ctx.Err() != nil { retryable = false }
	...
	if response == nil { return nil, usage, errors.New("quality provider returned no response") }
	if u := response.Message.Usage; u != nil {
		usage.InputTokens, usage.OutputTokens = u.Input, u.Output
		if u.Provider != "" { usage.Provider = u.Provider }
		if u.Model != "" { usage.Model = u.Model }
	}
	...
	if response.Message.HasToolCalls() { return nil, usage, errors.New("quality services do not allow tool execution") }
	text := strings.TrimSpace(response.Message.TextContent())
	if text == "" { return nil, usage, errors.New("quality provider returned empty text") }
...
func schemaFor(t reflect.Type) map[string]any {
	if t == reflect.TypeOf(json.RawMessage{}) { return map[string]any{} }
	if t.Kind() == reflect.Pointer { return map[string]any{"anyOf": []any{schemaFor(t.Elem()), map[string]any{"type": "null"}}} }
	switch t.Kind() {
	case reflect.Struct:

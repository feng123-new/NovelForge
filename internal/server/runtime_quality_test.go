...
func TestConfiguredQualityHTTPAndHumanRevision(t *testing.T) {
	...
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if json.Unmarshal([]byte(data), &input) != nil {
			http.Error(w, "missing structured input", 400)
			return
		}
		text := "张三来到山门，钟声响起。"
		switch input.Schema {
		case "FactProposal":
			...
			text = string(raw)
		case "EditorReview":
			raw, _ := json.Marshal(qualitygate.EditorReview{Score: 8.5, Summary: "clear and consistent"})
			text = string(raw)
		default:
			writerCalls.Add(1)
	...
	if !app.qualityConfigured(created.ID) {
		t.Fatal("project config not connected")
	}
	...
	if writerCalls.Load() != 1 || !sawContext.Load() || replay.Header().Get("Idempotency-Replayed") != "true" {
		t.Fatal("compiled context / paid-call replay broken")
	}
	...
	if err := json.Unmarshal(saved.Body.Bytes(), &revision); err != nil || revision.ID == "" {
		t.Fatalf("revision=%s err=%v", saved.Body.String(), err)
	}
	...
	restarted.Handler().ServeHTTP(state, httptest.NewRequest(http.MethodGet, base, nil))
	var view struct {
		ActiveFinal *chapterversion.Version `json:"active_final"`
	}
	if err := json.Unmarshal(state.Body.Bytes(), &view); err != nil || state.Code != 200 || view.ActiveFinal == nil || view.ActiveFinal.Content != humanText {
		t.Fatalf("restarted state=%s err=%v", state.Body.String(), err)
	}
}

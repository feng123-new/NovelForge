...
func Export(book Book,format string) ([]byte,string,error) {
 if len(book.Chapters)==0 || len(book.Chapters)>1000 {return nil,"",ErrInvalid}
 total:=0
 for _,c:=range book.Chapters {
  total+=len(c.Text)
  if !utf8.ValidString(c.Text) || strings.ContainsRune(c.Text,0) || c.Number<1 || strings.TrimSpace(c.Text)=="" {return nil,"",ErrInvalid}
  if len(c.Text)>MaxChapter || total>MaxExpanded {return nil,"",ErrLimit}
 }
 switch format {
 case "txt","md":
  var b strings.Builder
  for _,c:=range book.Chapters {
   if format=="md" { b.WriteString("# ") };b.WriteString(c.Title);b.WriteString("\n\n");b.WriteString(c.Text);b.WriteString("\n\n")
  }
  media:="text/plain; charset=utf-8";if format=="md" {media="text/markdown; charset=utf-8"}
  return []byte(b.String()),media,nil
 case "epub": return exportEPUB(book)
 default:return nil,"",ErrInvalid
 }
}
func exportEPUB(b Book) ([]byte,string,error) {
 var out bytes.Buffer;z:=zip.NewWriter(&out)
 put:=func(name,text string,method uint16) error {h:=&zip.FileHeader{Name:name,Method:method};w,e:=z.CreateHeader(h);if e!=nil{return e};_,e=io.WriteString(w,text);return e}
 if err:=put("mimetype","application/epub+zip",zip.Store);err!=nil{return nil,"",err}
 ...
 esc:=html.EscapeString;lang:=b.Language;if lang==""{lang="zh"}
 var manifest,spine,nav strings.Builder
 for i,c:=range b.Chapters {
  name:=fmt.Sprintf("chapter-%04d.xhtml",i+1);id:=fmt.Sprintf("c%d",i+1)
  fmt.Fprintf(&manifest,`<item id="%s" href="%s" media-type="application/xhtml+xml"/>`,id,name);fmt.Fprintf(&spine,`<itemref idref="%s"/>`,id)
  fmt.Fprintf(&nav,`<li><a href="%s">%s</a></li>`,name,esc(c.Title))
  var paragraphs strings.Builder;for _,line:=range strings.Split(strings.ReplaceAll(c.Text,"\r\n","\n"),"\n") {if strings.TrimSpace(line)!="" {paragraphs.WriteString("<p>"+esc(line)+"</p>")}}
 ...
 for _,name:=range names {if err:=put(name,files[name],zip.Deflate);err!=nil{return nil,"",err}}
 if err:=z.Close();err!=nil{return nil,"",err};return out.Bytes(),"application/epub+zip",nil
}

...
func TestLegacySelectionReturnsOnlyCompiledRecords(t *testing.T) {
	...
	if memory["chapter_plan"] != "must remain" {
		t.Fatal("mandatory plan dropped")
	}
	if _, exists := memory["style_reference"]; exists {
		t.Fatal("unselected raw field escaped budget")
	}
	if _, exists := raw["working_memory"].(map[string]any)["style_reference"]; !exists {

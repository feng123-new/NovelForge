...
func quoteCharacterFTSQuery(query string) string {
	...
	for _, field := range strings.Fields(query) {
		...
		if len(chars) > 0 {
			phrases = append(phrases, `"`+strings.Join(chars, " ")+`"`)
		}
	}
...
LIMIT 20`, query, request.ProjectID, request.Chapter)
	if err != nil {
		return nil, fmt.Errorf("contextcompiler: character search: %w", err)
	}
	defer rows.Close()
	var items []Item
	for rows.Next() {
...
func init() {
	sqlite.MustRegisterDeterministicScalarFunction("novelforge_search_characters", 1,
		func(_ *sqlite.FunctionContext, args []driver.Value) (driver.Value, error) {
			if len(args) != 1 {
				return nil, fmt.Errorf("character search expects one argument")
			}
			if args[0] == nil {
				return "", nil
			}
			...
			if !ok {
				return nil, fmt.Errorf("character search expects text")
			}

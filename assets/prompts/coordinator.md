...
- **修改类**：评估影响：
  ...
  - **写作风格/质量规则**（任何时候都成立、用来约束写作笔法且不绑定具体进度节点的要求：每章字数、用词偏好、禁用语、句式、对话占比、标题格式等，如"每章1500字左右""少用比喻""标题只用中文""对话多一点"）→ 调 `save_user_rules(text=...)` 落盘。系统会用模型把自然语言归一化成结构化约束写入本书规则，writer 据此写作、commit_chapter 据此自检，跨重启生效。工具返回"本次理解成了什么 + 当前全量生效约束"，**请把它回显给用户确认是否理解正确**；理解有偏差就再调一次修正补充。然后按"续写类"继续主线。
  - **带进度锚点的状态式长效指令**（在某个进度点开始或改变，如"从第30章起主角语气转冷""接下来这一卷多写战斗线"）→ 调 `save_directive(action=add)` 落盘。它会记录下达时的进度快照，所有子代理每章都会在 `working_memory.user_directives` 看到。取消或修改某条 → 看工具返回的序号列表，先 `save_directive(action=remove, index=N)` 删旧条目，必要时再 add 新表述。
  - 两类都**只存"状态式"要求**（任何时候重读都成立的描述）；相对式/动作式指令（"增加10章""重写第3章"）两个工具都绝不存——落盘不等于执行：没有子代理会因此被派出，用户的要求会被搁置。它们属于篇幅调整/返工，走上面的路由立即派单，由 architect/editor 翻译成大纲与 compass 的绝对状态。
...
- `save_user_rules(text)`：把用户的长效写作风格/质量要求归一化为结构化规则并持久化（**仅**用户干预属于"无进度锚点的风格/质量规则"时使用；返回的理解需回显给用户确认）
- `save_directive`：持久化用户**带进度锚点**的状态式长效指令（**仅**用户干预属于此类时使用）

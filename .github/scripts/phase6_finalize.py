...
old = "go test -race -buildvcs=false -count=1 ./internal/db/migrate ./internal/project ./internal/qualitygate ./internal/truthstore ./internal/server/..."
new = "go test -race -buildvcs=false -count=1 ./internal/db/migrate ./internal/project ./internal/qualitygate ./internal/truthstore ./internal/narrativeledger ./internal/server/..."
if old in ci_text:
    ci_text = ci_text.replace(old, new, 1)
elif "./internal/narrativeledger" not in ci_text:
    raise RuntimeError("could not locate targeted race command")
ci.write_text(ci_text, encoding="utf-8")

write("web/src/pages/Foreshadows.test.ts", r'''
...
describe('Foreshadows page', () => {
  ...
  it('renders the authoritative empty state', async () => {
    ...
    render(Foreshadows);
    expect(await screen.findByText('当前筛选没有伏笔')).toBeInTheDocument();
  });
...
write("web/src/pages/Secrets.test.ts", r'''
import { fireEvent, render, screen, waitFor } from '@testing-library/svelte';
import { afterEach, describe, expect, it, vi } from 'vitest';
...
describe('Secrets page', () => {
  ...
  it('renders the authoritative empty state', async () => {
    ...
    render(Secrets);
    expect(await screen.findByText('尚无 Secret')).toBeInTheDocument();
  });
  ...
  it('adds a temporal holder and publicly reveals through server writes', async () => {
    ...
    const { default: Secrets } = await import('./Secrets.svelte');
    render(Secrets);
    expect(await screen.findByText("The heir's origin")).toBeInTheDocument();
    ...
    const revealButton = screen.getByRole('button', { name: 'Public Reveal' });
...
append_once("docs/ARCHITECTURE.md", "## 18. Phase 6 Narrative Ledger", r'''
...
append_once("docs/TRUTH_STORE.md", "## Narrative Ledger integration", r'''
...
append_once("docs/WEB.md", "## Phase 6 Narrative Ledger pages", r'''
...
write("docs/IMPLEMENTATION_STATUS.md", r'''

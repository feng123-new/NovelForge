...
}catch(error){console.error('BROWSER_LAST_VIEW', await evaluate("JSON.stringify({url:location.href,title:document.title,heading:document.querySelector('main h1')?.textContent,text:document.body.innerText.slice(0,2000)})").catch(()=>''));writeFileSync(path.join(output,'failure.json'),JSON.stringify({error:String(error),exceptions,httpErrors,routes,requests:requests.slice(-30)},null,2));throw error;}

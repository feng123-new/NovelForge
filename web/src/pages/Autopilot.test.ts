...
describe('Autopilot controls', () => {
 it('shows persisted review state and routes approval through an idempotent write', async () => {
  ...
  const job = { id: 'job_test', revision: 7, review_candidate_id: 'candidate_1', project_id: 'p1', state: 'paused', stage: 'finalize', chapter: 1, completed_through: 0, target_chapter: 2, error_code: 'REVIEW_REQUIRED', actions: { pause: false, resume: true, stop: true } };
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
   const url = String(input); requests.push({ url, init });
   let data: unknown;
   if (url.includes('/autopilot/job_test/resume')) data = { job };
   else if (url.includes('/autopilot/job_test')) data = { job, candidate_id: 'candidate_1', candidate_text: 'A reviewed chapter.', chapter_plan: {}, foundation: {} };
  ...
  expect(JSON.parse(String(write.init?.body))).toEqual({ expected_revision: 7, review_candidate_id: 'candidate_1' });
  view.unmount();
 });
});

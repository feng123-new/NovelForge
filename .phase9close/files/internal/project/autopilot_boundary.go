...
func (r *Repository) autopilotDatabase(ctx context.Context,id string) (*sql.DB,error) {
 ...
 if err!=nil{return nil,err}; if err=versions.Close();err!=nil{return nil,err}
...
func (r *Repository) AutopilotFinalComplete(ctx context.Context,id string,chapter int) (bool,error) {
 if chapter<1{return false,ErrValidation}
 db,err:=r.autopilotDatabase(ctx,id);if err!=nil{return false,err};defer db.Close()
 var n int;var version,hash string
 err=db.QueryRowContext(ctx,autopilotCompletedFinalSQL+` AND a.chapter=?`,id,chapter).Scan(&n,&version,&hash)
 if err==sql.ErrNoRows{return false,nil};return err==nil,err
}
...
func (r *Repository) AutopilotNextChapter(ctx context.Context,id string) (int,error) {
 db,err:=r.autopilotDatabase(ctx,id);if err!=nil{return 0,err};defer db.Close()
 rows,err:=db.QueryContext(ctx,autopilotCompletedFinalSQL+` ORDER BY a.chapter LIMIT 1001`,id)
 if err!=nil{return 0,err};defer rows.Close()
 next:=1
 for rows.Next(){var n int;var version,hash string;if err=rows.Scan(&n,&version,&hash);err!=nil{return 0,err};if n!=next{break};next++}
...
func (r *Repository) AutopilotFingerprint(ctx context.Context,id string,chapter int) (string,error) {
 if chapter<1||chapter>1000{return "",ErrValidation}
 db,err:=r.autopilotDatabase(ctx,id);if err!=nil{return "",err};defer db.Close()
 tx,err:=db.BeginTx(ctx,&sql.TxOptions{ReadOnly:true});if err!=nil{return "",err};defer tx.Rollback()
 ...
 h:=sha256.New();encoder:=json.NewEncoder(h)
 if err=encoder.Encode([]any{id,chapter,truth,foreshadows,secrets});err!=nil{return "",err}
 ...
 if err!=nil{return "",err}
 for rows.Next(){var n int;var version,authority string;if err=rows.Scan(&n,&version,&authority);err!=nil{rows.Close();return "",err};if err=encoder.Encode([]any{n,version,authority});err!=nil{rows.Close();return "",err}}
 ...
 if err!=nil{return "",err};if incomplete>0{return "",fmt.Errorf("accepted state has an incomplete rebuild")}

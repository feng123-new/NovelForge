...
func TestServerConfigFlagParsesBeforeUnexpectedArguments(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if code != 2 || !strings.Contains(string(data), "unexpected arguments") || strings.Contains(string(data), "flag provided but not defined") { t.Fatalf("exit=%d stderr=%s", code, data) }
}
